...
-  **`Mode`** (Default: `"biome"`) — Determines how the type of Evil Mimic is chosen. Available modes:
   -  `default`: Uses the evil type the world can naturally spawn (Corrupt or Crimson). Recommended for the 'getfixedboi' seed, where the Evil Mimic type changes each day. Can be summoned anywhere.
